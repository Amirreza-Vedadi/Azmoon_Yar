...
class QuestionsUI:
    ...
    def test(self, selected):
        ...
        match = re.search(r'\d+', selected[6])
        if match:
            correct_option_number = int(match.group())
        else:
            correct_option_number = 1
        correct_option_text = f"گزینه {correct_option_number}"
        self.correct_option_cmb.set(correct_option_text)
    ...
    def search_question(self):
        ...
        if question and isinstance(question, (list, tuple)):
            self.clear()
            ...
            try:
                correct_option_number = int(question[6])
                correct_option_text = f"گزینه {correct_option_number}"
            except (ValueError, TypeError):
                correct_option_text = question[6]
            self.correct_option_cmb.set(correct_option_text)
    ...
    def save_question(self):
        ...
        correct_option_text = self.correct_option_cmb.get()
        exam_time = self.exam_time_txt.get()
        navigation_enabled = self.navigation_enabled_var.get()

        if not question or not option1 or not correct_option_text or not exam_time:
            showerror("خطا", "لطفاً تمام فیلدها را پر کنید")
            return

        q_controller = QuestionsController()
        result = q_controller.save(question, option1, option2, option3, option4, correct_option_text, exam_time,
                                   navigation_enabled)
    ...
    def edit_question(self):
        ...
        q_controller = QuestionsController()
        result = q_controller.edit(self.selected_id, question, option1, option2, option3, option4, correct_option_text,
    ...
    def convert_option_to_number(self, option_text):
        """تبدیل مقدار متنی گزینه صحیح به عدد"""
        options_map = {"گزینه ۱": 1, "گزینه ۲": 2, "گزینه ۳": 3, "گزینه ۴": 4}
        return options_map.get(option_text, 1)
    ...
    def refresh_table(self):
        ...
        for question in questions_list:
            try:
                ...
                correct_option_text = f"گزینه {correct_option_value}"
            except (ValueError, TypeError):
                correct_option_text = question[6]
            updated_question = list(question)
            updated_question[6] = correct_option_text
            updated_questions.append(updated_question)
        self.table.set_items(updated_questions)
